...
const calculateScore = frames => {
    ...
    for (let i in framesObjectArray) {
        ...
        for (let j in frame.throws) {
            if (frame.throws[j] === "X") {
                ...
                frame.inFrameTotal +=
                    10 - parseInt(frame.throws[parseInt(j) - 1]);
            } else {
    ...
    for (let i in framesObjectArray) {
        ...
        for (let j in frame.throws) {
            if (frame.throws[j] === "X") {
                const nextFrameThrows =
                    framesObjectArray[parseInt(i) + 1].throws;

                //if next frame has only one throw, it is a strike, add that and the following throw
                if (nextFrameThrows.length === 1) {
                    frame.bonusTotal += 10;

                    const followingFrameFirstThrow =
                        framesObjectArray[parseInt(i) + 2].throws[0];

                    frame.bonusTotal +=
                        followingFrameFirstThrow === "X"
                            ? 10
                            : parseInt(followingFrameFirstThrow);
                } else {
                ...
                if (nextThrow) {
                    ...
                    nextFrameFirstThrow =
                        framesObjectArray[parseInt(i) + 1].throws[0];
                    frame.bonusTotal +=
                        nextFrameFirstThrow === "X" ? 10 : parseInt(nextFrameFirstThrow);
                }
            }
        }
    }
    ...
    framesObjectArray.forEach(
        frame => (totalScore += frame.inFrameTotal + frame.bonusTotal)
    );
